...
Tape: Picture an infinitely long strip of paper (the tape) divided into small squares. Each square can hold a symbol (like a 0 or 1).
Head: There is a read/write head that moves along the tape, one square at a time. It can read the symbol in the current square, write a new symbol, and then move left or right.
State: The machine is always in one of a finite number of states. Based on the current state and the symbol it reads, it will write a new symbol, move the head, and switch to a new state according to a set of rules (the program).
...
All it needs to do is anything that can processes data held on your computer by a given set of instructions and algorithms